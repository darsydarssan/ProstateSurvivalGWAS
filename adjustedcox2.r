##############################################################################################################
#Purpose: Adjusted significan SNPs p-values. Identify the SNP factor which has p-value less than the threshold
##############################################################################################################
...
datsnp <- dat[c("Onc_ID", as.character(snpnam$snp), "DateDiag", "DateLastF", "VitalStatus", "AgeDiag", "pc1_euro", "pc2_euro",	
                "pc3_euro",	"pc4_euro",	"pc5_euro",	"pc6_euro",	"pc7_euro")]
datsnp$AgeDiag <- suppressWarnings(as.numeric(as.character(datsnp$AgeDiag)))
...
cphform <- lapply(colnames(datsnp[2:(which(names(datsnp)=="DateDiag")-1)]), 
...
coef1 <- t(t(unlist(lapply(model, function(x){x$coef[1:(length(x$coef)-8)]}))))
se1 <- t(t(unlist(lapply(model, function(x){sqrt(diag(x$var)[1:(length(diag(x$var))-8)])}))))
pval1 <- t(t(unlist(lapply(model, function(x){(pnorm(abs(x$coef/sqrt(diag(x$var))),lower.tail=F)*2)[1:(length(pnorm(abs(x$coef/sqrt(diag(x$var))),lower.tail=F)*2)-8)]})))) 
...
adjsnppval <- data.frame(ChrNo=finchrdat$Chr, SNPName=finchrdat$snpnam, Genotype=finchrdat$genotypeall, Frequency=round(as.numeric(as.character(finchrdat$freqall)),4), 
...
adjsnppvalout <- regulartable(data=adjsnppval)
adjsnppvalout <- align(adjsnppvalout, align="left", part="all")
adjsnppvalout <- set_formatter(adjsnppvalout, "HR"=double_format, "pval"=pval_format)
adjsnppvalout <- theme_box(adjsnppvalout)

#Export to my word report
report <- report %>% 
  cursor_reach(keyword="pleasedohereadjustpval") %>%
  body_add_flextable(value=adjsnppvalout, align="left", split=T)
